...
export const FeaturedCard = ({id, property }: Props) => {
  ...
  return (
    ...
    >    
        <Image source={{ uri: property.thumbnailImage as string }} className='size-full rounded-2xl' />
        <Image source={images.cardGradient} className='size-full rounded-2xl absolute bottom-0' />

        <View className='flex flex-row items-center bg-white/90 px-3 py-1.5 rounded-full absolute top-5 right-5'>
            <Image source={icons.star} className='size-3.5' />
            <Text className='text-xs font-rubik-bold text-primary-300 ml-1'>{property?.averageRating?.toFixed(1)}</Text>
        </View>
        ...
        <View className='flex flex-col items-start absolute bottom-5 inset-x-5'>
            ...
            <View className='flex flex-row items-center justify-between w-full'>
                <Text className='text-xl font-rubik-extrabold text-white'>${property.price}</Text>
